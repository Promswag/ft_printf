...
/*   Updated: 2022/03/16 16:38:08 by gbaumgar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	ft_printf_format_list(va_list *args_list, int format)
{
	if (format == 'c')
		...
		return (ft_printf_display_ptr(va_arg(*args_list, void *)));
...
int	ft_printf(const char *args, ...)
{
	...
	if (args)
	{
		...
		{
			if (*args == '%')
				cc += ft_printf_format_list(&args_list, *(args++ + 1));
			else
